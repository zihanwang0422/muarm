...
RADIUS_Y  = 0.12   # half-width  in Y direction (m)
RADIUS_Z  = 0.08   # half-height in Z direction (m)
...
class TrajectoryRunner:
    ...
    def _draw_overlay(self, viewer, ref_pts):
        """Draw the reference figure-8 outline (orange) and EE trail (cyan)."""
        ee = self.data.body("hand").xpos.copy()
        if not self._trail or np.linalg.norm(ee - self._trail[-1]) > 0.002:
        ...
        # Reference figure-8 outline  (closed loop)
        for i in range(n_ref):
            if ngeom >= viewer.user_scn.maxgeom:
                break
            g = viewer.user_scn.geoms[ngeom]
            mujoco.mjv_connector(
                g, mujoco.mjtGeom.mjGEOM_LINE, 2.0,
                np.array(ref_pts[i],           dtype=np.float64),
                np.array(ref_pts[(i+1) % n_ref], dtype=np.float64),
            )
            g.rgba[:] = np.array([1.0, 0.6, 0.1, 0.7], dtype=np.float32)
            ngeom += 1
    ...
    def run(self):
        ...
        with mujoco.viewer.launch_passive(self.model, self.data) as viewer:
            ...
            while viewer.is_running():
                self._trail = []          # clear trail each new loop
                for q in positions:
                    ...
                    self._draw_overlay(viewer, fig8_xyz)
                    viewer.sync()
