...
class MuJoCoViewer:
    ...
    def add_visual_lines(self, segments, width=3.0, rgba=None):
        """Draw line segments in the scene overlay using mjv_connector.

        Args:
            segments: list of (from_pos, to_pos) pairs, each shape (3,)
            width: line width in pixels
            rgba: [r, g, b, a] colour applied to all segments
        """
        if not segments:
            return
        rgba_f = np.array(rgba or [1.0, 0.2, 0.2, 0.9], dtype=np.float32)
        cur = self.handle.user_scn.ngeom
        n   = min(len(segments), self.handle.user_scn.maxgeom - cur)
        if n <= 0:
            return
        self.handle.user_scn.ngeom = cur + n
        for i, (p0, p1) in enumerate(segments):
            g = self.handle.user_scn.geoms[cur + i]
