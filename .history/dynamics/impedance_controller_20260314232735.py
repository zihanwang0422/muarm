...
class FrankaImpedanceDemo(MuJoCoViewer):
    ...
    KP = np.array([300.0, 600.0, 600.0, 30.0, 30.0, 30.0])
    KD = np.array([50.0, 100.0, 100.0, 2.0, 2.0, 2.0])
    ...
    def runFunc(self):
        ...
        if self._phase == "float":
            ...
            # Debug: 打印关节级误差与力矩
            if np.linalg.norm(error) > 0.05: # 有明显误差时才打印
                print(f"[Debug] e_pos: {np.round(e_pos, 3)}, e_rot: {np.round(e_rot, 3)}")
                print(f"[Debug] F_cartesian: {np.round(F_cartesian, 2)}")
                print(f"[Debug] tau_imp: {np.round(tau_imp, 2)}")
                print(f"[Debug] ee_vel: {np.round(ee_vel, 3)}\n")
